...
<script type="text/javascript">
	...
	export default {
		...
		methods: {
			...
			exportExcel(){
				window.location.href = 'http://localhost:8081/admin/order/export'
			},
			updateData(){
			...
			deliver(row){
				this.$confirm('确认更新状态至发货?', '发货确认', {
		          ...
		          let query = {id: row.id, status: 3, userId: row.userId, orderId: row.orderId, packupCode: row.packupCode}
		          console.log(query)
